struct ImplicitProjectionDynamics{T,R,RZ,Rθ} <: Model{T}
    n::Int
    m::Int
    d::Int
	ip_dyn::InteriorPoint{T,R,RZ,Rθ} 
	eval_proj::InteriorPoint{T,R,RZ,Rθ} 
	grad_proj::InteriorPoint{T,R,RZ,Rθ}
	f_tmp::Vector{T} 
	fx_tmp::Matrix{T} 
	fu_tmp::Matrix{T}
	q1_idx::Vector{Int} 
...
function ImplicitDynamics(model, h, r_func_dyn, rz_func_dyn, rθ_func_dyn; 
